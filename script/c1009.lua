...
function c1009.initial_effect(c)
	--Fusion Material
	aux.AddFusionProcCodeFun(c,1006,aux.FilterBoolFunction(Card.IsFusionSetCard,0x3),1,true,true)
	c:EnableReviveLimit()
	--Set Card
	...
	e3:SetCondition(c1009.condition2)
	e3:SetValue(c1009.value)
	c:RegisterEffect(e3)
end
...
function c1009.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c1009.filter,tp,LOCATION_DECK,0,1,nil) end
end
...
function c1009.value(e,re,tp)
	return not re:GetHandler():IsImmuneToEffect(e)
end
function c1009.condition2(e)
