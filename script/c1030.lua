...
function c1030.initial_effect(c)
	...
	e2:SetCondition(c1030.condition)
	e2:SetTarget(c1030.target)
	e2:SetOperation(c1030.operation)
	c:RegisterEffect(e2)
	--Draw
	...
	e3:SetCountLimit(1)
	e3:SetCost(c1030.cost)
	e3:SetTarget(c1030.target2)
	e3:SetOperation(c1030.operation2)
	c:RegisterEffect(e3)
end
...
function c1030.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1030.filter,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,c1030.filter,1,1,REASON_COST+REASON_DISCARD)
end
