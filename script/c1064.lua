...
function c1064.initial_effect(c)
	...
	e1:SetCondition(c1064.condition)
	e1:SetTarget(c1064.target)
	e1:SetOperation(c1064.operation)
	c:RegisterEffect(e1)
end
function c1064.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x1c)
end
function c1064.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c1064.filter,tp,LOCATION_MZONE,0,1,nil) and Duel.GetLP(tp)<=1000
end
