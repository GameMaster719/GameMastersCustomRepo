...
function c335599171.initial_effect(c)
	--discard both players hands when flipped
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_HANDES+CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_FLIP)
	e1:SetTarget(c335599171.target)
	e1:SetOperation(c335599171.operation)
	c:RegisterEffect(e1)
	--Crush Terrain
...
c335599171.collection={ [511005597]=true; [7914843]=true; [5257687]=true; [75285069]=true; [39180960]=true; [70307656]=true; [2792265]=true; [4035199]=true; [78636495]=true; [44913552]=true; [31242786]=true;  }


function c335599171.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,0,PLAYER_ALL,0)
end

function c335599171.operation(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetFieldGroup(tp,LOCATION_HAND,LOCATION_HAND)
	if g:GetCount()>0 then Duel.SendtoGrave(g,REASON_DISCARD+REASON_EFFECT) end
end

function c335599171.ct_check(c)
	return c:GetAttack()>=1500 and not c335599171.collection[c:GetCode()]
end


function c335599171.filter2(c)
...
function condition2(e)
  return
   not c335599171.filter2(Duel.GetFieldCard(0,LOCATION_SZONE,5)) and
   not c335599171.filter2(Duel.GetFieldCard(1,LOCATION_SZONE,5))
end
...
function desop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if seq>0 then
        ...
        if tc and c335599171.ct_check(tc) then g:AddCard(tc) end
    end
    if seq<4 then
        local tc=Duel.GetFieldCard(tp,LOCATION_MZONE,seq+1)
       if tc and c335599171.ct_check(tc) then g:AddCard(tc) end
