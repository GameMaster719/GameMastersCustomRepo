...
return not c:IsHasEffect(EFFECT_NECRO_VALLEY) and c:IsType(TYPE_MONSTER)
end
...
return not c:IsHasEffect(EFFECT_NECRO_VALLEY) and (c:IsDestructable() and c:IsAbleToGrave() and c:IsType(TYPE_MONSTER+TYPE_SPELL+TYPE_TRAP)) 
end
...
--standby matnience cost effect 
function c33559994.mtcon(e,tp,eg,ep,ev,re,r,rp)
return Duel.GetTurnPlayer()==tp
end
function c33559994.mtop(e,tp,eg,ep,ev,re,r,rp)
if Duel.GetLP(tp)>1200 and Duel.SelectYesNo(tp,aux.Stringid(33559994,2)) then
