...
function c33569919.filter(c)
	return c:IsType(TYPE_MONSTER) and c:IsFaceup()
end

function c33569919.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp)  end
	if chk==0 then return Duel.IsExistingTarget(c33569919.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c33569919.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
