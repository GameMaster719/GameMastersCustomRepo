...
function c1006.initial_effect(c)
	...
	e1:SetCondition(c1006.condition)
	e1:SetTarget(c1006.target)
	e1:SetOperation(c1006.operation)
	c:RegisterEffect(e1)
	--Direct Attack
...
function c1006.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c1006.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1006.filter,tp,0,LOCATION_GRAVE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c1006.filter,tp,0,LOCATION_GRAVE,1,1,nil)
