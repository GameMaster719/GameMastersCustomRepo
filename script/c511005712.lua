...
return c:IsFaceup() and c:IsCode(44944304) and c:GetAttack()~=val 
end
...
--Atk Value change condition
function c511005712.condition(e,tp,eg,ep,ev,re,r,rp)
if Duel.GetMatchingGroupCount(c511005712.rcfilter,tp,0,LOCATION_MZONE,nil)==0 or Duel.GetMatchingGroupCount(c511005712.cfilter0,tp,LOCATION_MZONE,0,nil)==0 then return false end
local c=e:GetHandler()
local tp=c:GetControler()
return (eg:IsExists(function (c) return c:GetFlagEffect(284)>0  and c:GetFlagEffectLabel(284)~=0 and c:IsControler(1-e:GetHandlerPlayer())  end,1,e:GetHandler(),tp) 
 and (Duel.GetCurrentPhase()>=PHASE_DRAW and Duel.GetCurrentPhase()<=PHASE_BATTLE)) or  (eg:IsExists(c511005712.cfilter0,1,nil,e:GetHandler())  and (Duel.GetCurrentPhase()>=PHASE_DRAW and Duel.GetCurrentPhase()<=PHASE_BATTLE) and  re and re:IsActiveType(TYPE_SPELL+TYPE_TRAP) or (re:IsActiveType(TYPE_MONSTER) and not re==e:GetHandler() ))
end
...
local c=e:GetHandler()
local ct1=Duel.GetMatchingGroupCount(c511005712.rcfilter,tp,0,LOCATION_MZONE,nil)
local ct2=Duel.GetMatchingGroupCount(c511005712.cfilter0,tp,LOCATION_MZONE,0,nil)
if  ct1>0 and ct2>0 then
local ct3=ct1+ct2
local e1=Effect.CreateEffect(c)
e1:SetType(EFFECT_TYPE_SINGLE)
e1:SetCode(EFFECT_UPDATE_ATTACK)
e1:SetValue(1000*ct3)
