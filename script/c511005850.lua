...
function c33569932.initial_effect(c)
	--Activate
	...
	e1:SetTarget(c33569932.target)
	e1:SetOperation(c33569932.activate)
	c:RegisterEffect(e1)
end
...
function c33569932.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	e2:SetOperation(c33569932.desop)
...
function c33569932.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local tc=Duel.GetFirstTarget()
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c33569932.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c33569932.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,tc) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c33569932.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,tc)
end


function c33569932.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local bc=c:GetBattleTarget()
