...
function c33569926.initial_effect(c)
    ...
    e1:SetCondition(c33569926.con)
    e1:SetOperation(c33569926.op)
    e1:SetProperty(EFFECT_FLAG_SET_AVAILABLE)
    c:RegisterEffect(e1)
    local e2=Effect.CreateEffect(c)
    e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_FLIP)
    e2:SetCategory(CATEGORY_CONTROL)
    e2:SetTarget(c33569926.target)
    e2:SetOperation(c33569926.control)
    c:RegisterEffect(e2)
    e1:SetLabelObject(e2)
end
function c33569926.con(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():IsPosition(POS_FACEDOWN)
end

function c33569926.op(e,tp,eg,ep,ev,re,r,rp)
