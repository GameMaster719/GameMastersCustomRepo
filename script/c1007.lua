...
function c1007.initial_effect(c)
	...
	e2:SetTarget(c1007.target2)
	e2:SetOperation(c1007.operation2)
	c:RegisterEffect(e2)
end
...
function c1007.filter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsDestructable()
end
function c1007.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c1007.filter,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c1007.filter,tp,0,LOCATION_ONFIELD,1,1,nil)
...
function c1007.target2(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c1007.operation2(e,tp,eg,ep,ev,re,r,rp)
