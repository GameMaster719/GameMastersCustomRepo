...
--to grave
	...
	e0:SetOperation(c335599103.regop2)
	c:RegisterEffect(e0)
end

function c335599103.regop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsReason(REASON_DESTROY) and bit.band(c:GetPreviousLocation(),LOCATION_ONFIELD)~=0 then
		--spsummon
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringid(335599103,0))
		e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
		e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
		e1:SetRange(LOCATION_GRAVE)
		e1:SetCode(EVENT_PHASE+PHASE_END)
		e1:SetCost(c335599103.spcost)
		e1:SetTarget(c335599103.sptg)
		e1:SetOperation(c335599103.spop)
...
function c335599103.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:GetFlagEffect(335599103)==0 end
	c:RegisterFlagEffect(335599103,nil,0,1)
end

function c335599103.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end

function c335599103.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
