...
function c33559922.filter2(c)
return not c:IsHasEffect(EFFECT_NECRO_VALLEY) and ((c:IsAbleToHand() and c:IsType(TYPE_TOON)  or c:IsSetCard(0x62) or c:IsCode(99261403) or c:IsCode(62762898) ) )
end

function c33559922.condition(e,c)
return Duel.IsExistingMatchingCard(c33559922.filter2,e:GetHandler():GetControler(),LOCATION_DECK+LOCATION_GRAVE,0,1,nil)
end
function c33559922.operation(e,tp,eg,ep,ev,re,r,rp,chk)	
Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOHAND)
local g=Duel.SelectMatchingCard(tp,c33559922.filter2,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
...
return c:IsCode(33559923) and c:IsAbleToGrave()
end
